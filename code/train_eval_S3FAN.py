...
from S3FAN_wanzheng import SpectralFeatureExtractor  # 使用新的 FECNN 模型
import os
...
def train(modelConfig: Dict):
    ...
    # Model setup
    input_channels = modelConfig["band"]
    dim = modelConfig["channel"]
    depth = modelConfig["depth"]
    heads = modelConfig["heads"]
    dim_head = modelConfig["dim_head"]
    dropout = 0.1

    net_model = SpectralFeatureExtractor(
        input_channels=input_channels, dim=dim, depth=depth, heads=heads, dim_head=dim_head, dropout=dropout
    ).to(device)
    ...
    for e in range(modelConfig["epoch"]):
        ...
        with tqdm(dataloader, dynamic_ncols=True) as tqdmDataLoader:
            for positive, negative in tqdmDataLoader:
                # Combine positive, negative, and target spectrum
                ...
                optimizer.zero_grad()
                features = net_model(combined_vectors.unsqueeze(1))  # Adjusted for 1D Conv
...
def eval(modelConfig: Dict):
    ...
    with torch.no_grad():
        ...
        target_spectrum_tensor = torch.from_numpy(target_spectrum.T).float().unsqueeze(1).to(device)
        target_features = model(target_spectrum_tensor).cpu().detach().numpy()

        # Process data in batches
        batch_size = modelConfig['batch_size']
        for i in range(0, numpixel - batch_size, batch_size):
            pixels = torch.from_numpy(data_matrix[i:i + batch_size]).float().unsqueeze(1).to(device)
            features = model(pixels).cpu().detach().numpy()
            detection_map[i:i + batch_size] = cosin_similarity_numpy(features, target_features)
        ...
        left_num = numpixel % batch_size
        if left_num != 0:
            pixels = torch.from_numpy(data_matrix[-left_num:]).float().unsqueeze(1).to(device)
            features = model(pixels).cpu().detach().numpy()
            detection_map[-left_num:] = cosin_similarity_numpy(features, target_features)

        # Reshape detection map
        detection_map = np.reshape(detection_map, [h, w], order='F')
        detection_map = standard(detection_map)
        detection_map = np.clip(detection_map, 0, 1)
