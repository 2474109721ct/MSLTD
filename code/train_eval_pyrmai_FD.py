...
from modle_pyramidtrans_FD import SpectralGroupAttention  # 修改为正确的导入路径
import os
...
def train(modelConfig: Dict):
    ...
    net_model = SpectralGroupAttention(
        ...
        num_layers=modelConfig['layer']  # 参数名称需与类定义一致
    ).to(device)
...
def select_best(modelConfig: Dict):
    ...
    for e in range(modelConfig['epoch']):
        with torch.no_grad():
            ...
            for i in range(0, numpixel, modelConfig['batch_size']):
                pixels = data_matrix[i:i + modelConfig['batch_size']]
