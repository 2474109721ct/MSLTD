...
def cosin_similarity(x, y):
    norm_x = np.sqrt(np.sum(x ** 2, axis=-1))   #  x的 L2 范数。
    norm_y = np.sqrt(np.sum(y ** 2, axis=-1))
    x_y = np.sum(np.multiply(x, y), axis=-1)
    similarity = np.clip(x_y / (norm_x * norm_y), -1, 1)  #x 和 y 的余弦相似度，
    return np.arccos(similarity)


## 这段代码根据 type 参数的不同值，选择不同的方法生成目标光谱，包括均值光谱、去偏后的均值光谱、中位数光谱，以及根据距离选择最近的光谱等方法。
...
# 生成不同程度的扰动光谱
def generate_perturbed_spectra(avg_target_spectrum, num_samples, epsilon_range=(0.01, 0.05)):
    target_samples = []
    for i in range(num_samples):
        epsilon = np.random.uniform(epsilon_range[0], epsilon_range[1])  # 随机扰动强度
        delta = epsilon * np.sign(np.random.randn(*avg_target_spectrum.shape))  # 扰动
        target_sample = avg_target_spectrum + delta
        target_sample = np.clip(target_sample, 0, 1)  # 限制光谱值在[0, 1]范围内
        target_samples.append(target_sample)
    return np.array(target_samples)

# 特征融合函数：将多个扰动后的样本进行融合
def feature_fusion(samples):
    # 简单的线性组合方式
    fused_sample = np.mean(samples, axis=0)  # 对多个样本取均值作为融合特征
    return fused_sample
