...
from MLSN import FECNN  # 使用新的 FECNN 模型
import os
...
def train(modelConfig: Dict):
    ...
    for e in range(modelConfig["epoch"]):
        ...
        #torch.save(net_model.state_dict(), os.path.join(path, f'ckpt_{e}.pt'))
...
def select_best(modelConfig: Dict):
    ...
    for e in range(modelConfig['epoch']):
        with torch.no_grad():
            ...
            model = FECNN()
            ckpt = torch.load(os.path.join(path, f"ckpt_{e}.pt"), map_location=device)
            ...
            target_features = model(
                torch.from_numpy(target_spectrum.T).float().unsqueeze(1).to(device)).cpu().detach().numpy()
